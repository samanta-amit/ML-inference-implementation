...
void inference_pipeline::process_nw_request(void){
...
void inference_pipeline::process_pr_request(void){
    while(1){
        std::unique_lock<std::mutex> pr_mutex_mgr(pr_queue_mutex);
...
void inference_pipeline::process_ld_request(void){
    while(1){
        std::unique_lock<std::mutex> ld_mutex_mgr(ld_queue_mutex);
...
void inference_pipeline::echo_input_back(int client_id, int model_id, std::string op_string, tcp_connection::pointer sp){
